...
class AnimationController {
    ...
    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number, inverted = false) {
        this.canvas = canvas
        this.ctx = ctx
        this.dpr = dpr
        this.size = size
    ...
    public render() {
        ...
        const fg = this.inverted ? '#1D1D1F' : '#ffffff'
    ...
    // 绘制轨迹
    private drawTrail(t1: number, fg = 'white') {
...
export function SpiralAnimation({ inverted = false }: { inverted?: boolean }) {
    ...
    useEffect(() => {
        ...
        // 创建动画控制器
        animationRef.current = new AnimationController(canvas, ctx, dpr, size, inverted)
